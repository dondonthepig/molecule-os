...
export type ReactionSearchSuggestion = { id: string; label: string; sublabel: string };

export function ReactionSearch({
  value,
  onChange,
  suggestions,
  onSelectSuggestion,
  ...
  suggestions: ReactionSearchSuggestion[];
    ...
    <div className={cn("relative", className)}>
      <div className="glass-subtle flex items-center gap-2.5 rounded-full border-border/60 px-4 py-2.5">
        ...
        <input
          ...
          placeholder={dict.reactionAtlas.search.placeholder}
        ...
        {value ? (
          <button
            type="button"
            onClick={() => onChange("")}
            aria-label={dict.reactionAtlas.search.clear}
      ...
      {showSuggestions ? (
        <div className="glass absolute inset-x-0 top-full z-20 mt-2 max-h-72 overflow-y-auto rounded-2xl border-border/60 p-2">
          <p className="px-2.5 py-1 text-[11px] font-semibold text-muted-foreground">
            {dict.reactionAtlas.search.suggestionsLabel}
          </p>
          {suggestions.map((s) => (
            <button
              key={s.id}
