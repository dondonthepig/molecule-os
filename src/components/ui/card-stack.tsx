...
export function CardStack<T extends CardStackItem>({
  ...
  // Clamp `active` back in bounds when the item count changes, computed
  // during render (per React's "adjusting state" guidance) instead of an effect.
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="pointer-events-none absolute inset-x-0 top-6 mx-auto h-48 w-[70%] rounded-full bg-brand-blue/5 blur-3xl"
          aria-hidden="true"
        />
        <div
          className="pointer-events-none absolute inset-x-0 bottom-0 mx-auto h-40 w-[76%] rounded-full bg-black/20 blur-3xl dark:bg-black/40"
          aria-hidden="true"
        />
            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 overflow-hidden rounded-2xl border-4 border-black/10 shadow-xl select-none will-change-transform dark:border-white/10",
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
            ...
            {items.map((it, idx) => {
              ...
              return (
                <button
                  ...
                  aria-label={`${dict.common.ariaGoTo} ${it.title}`}
                />
              );
            })}
          </div>
          {activeItem.href ? (
            <Link
              ...
              aria-label={dict.common.ariaOpenLink}
            >
